...
final class GroqAi
{
    ...
    {
        return getenv('GROQ_API_KEY') ? true : false;
    }
    ...
    public static function generateSmsReply(array $feedback): string
    {
        ...
        $brand = getenv('AI_BRAND_NAME') ?: 'our restaurant';
    ...
    {
        $brand = trim(getenv('AI_BRAND_NAME') ?: 'FeedbackHub');
        $name = trim((string) ($feedback['name'] ?? ''));
    ...
    {
        $primary = trim((string) getenv('GROQ_MODEL'));
        $chain = [];
        if ($primary !== '') {
            $chain[] = $primary;
        }
