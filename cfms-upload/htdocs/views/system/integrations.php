    ...
    <div class="col-md-6">
        <section class="panel">
            <div class="d-flex justify-content-between align-items-start mb-3">
                ...
                <span class="badge <?= $cfg('SMS_ENABLED', 'true') === 'true' ? 'bg-success' : 'bg-secondary' ?>"><?= $cfg('SMS_ENABLED', 'true') === 'true' ? 'Active' : 'Disabled' ?></span>
            </div>
            <dl class="row small mb-0">
                <dt class="col-4 text-muted">Endpoint</dt>
                <dd class="col-8"><?= Security::e($cfg('SMS_ENDPOINT', 'https://comms.egosms.co/api/v1/json/')) ?></dd>
                <dt class="col-4 text-muted">Sender</dt>
                <dd class="col-8"><?= Security::e($cfg('SMS_SENDER_ID', 'FeedbackHub')) ?></dd>
                <dt class="col-4 text-muted">Username</dt>
                <dd class="col-8"><?= Security::e($cfg('SMS_USERNAME', 'bagumajessysmith')) ?></dd>
            </dl>
        </section>
    </div>
    <div class="col-md-6">
        <section class="panel">
            <div class="d-flex justify-content-between align-items-start mb-3">
                ...
                <span class="badge <?= $cfg('GROQ_API_KEY') ? 'bg-success' : 'bg-secondary' ?>"><?= $cfg('GROQ_API_KEY') ? 'Connected' : 'Not configured' ?></span>
            </div>
            <dl class="row small mb-0">
                <dt class="col-4 text-muted">Model</dt>
                <dd class="col-8"><?= Security::e($cfg('GROQ_MODEL', 'llama-3.1-8b-instant')) ?></dd>
                <dt class="col-4 text-muted">API Key</dt>
                <dd class="col-8"><?= $cfg('GROQ_API_KEY') ? '••••' . substr($cfg('GROQ_API_KEY'), -4) : '—' ?></dd>
                <dt class="col-4 text-muted">Brand</dt>
                <dd class="col-8"><?= Security::e($cfg('AI_BRAND_NAME', 'FeedbackHub')) ?></dd>
            </dl>
        </section>
    </div>
    <div class="col-md-12">
